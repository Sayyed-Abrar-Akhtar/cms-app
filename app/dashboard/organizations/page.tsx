...
import { OrganizationsListClient, type OrganizationSummary } from "./_components/OrganizationsListClient";
...
export default async function OrganizationsListPage() {
  ...
  const serializedOrgs: OrganizationSummary[] = organizations.map((org) => {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background)] text-[var(--color-foreground)] font-mono p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        <OrganizationsListClient organizations={serializedOrgs} />
      </div>
    </div>
  );
}
